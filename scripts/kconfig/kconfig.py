...
# Writes/updates the rtochius/.config configuration file by merging configuration
# files passed as arguments, e.g. board *_defconfig and application prj.conf
# files.
#
# When fragments haven't changed, rtochius/.config is both the input and the
...
# rtochius doesn't use tristate symbols. They're supported here just to make the
...
def main():
    ...
    if args.rtochius_base:
        os.environ['RTOCHIUS_BASE'] = args.rtochius_base
    ...
    if args.handwritten_input_configs:
        ...
        # loading rtochius/.config, because rtochius/.config is configuration
...
def missing_deps(sym):
    ...
    # even though rtochius doesn't use them).
    #
...
def parse_args():
    ...
    parser.add_argument("--handwritten-input-configs",
                        ...
                        help="Assume the input configuration fragments are "
                             ...
                             "assigned")
    parser.add_argument("--rtochius-base",
                        help="Path to current rtochius installation")
