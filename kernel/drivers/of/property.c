				...
				const char *propname, int elem_size)
{
	...
	if (prop->length % elem_size != 0) {
		pr_err("size of %s in node %p is not a multiple of %d\n",
			    ...
			    struct of_endpoint *endpoint)
{
	...
	WARN_ONCE(!port_node, "%s(): endpoint %p has no parent node\n",
		  __func__, node);
					...
					struct device_node *prev)
{
	...
	if (!prev) {
		...
		if (!port) {
			pr_err("graph: no port node found in %p\n", parent);
			return NULL;
		}
	} else {
		port = of_get_parent(prev);
		if (WARN_ONCE(!port, "%s(): endpoint %p has no parent node\n",
			      __func__, prev))
			return NULL;
	}
					     ...
					     u32 port, u32 endpoint)
{
	...
	if (!endpoint_node) {
		pr_debug("no valid endpoint (%d, %d) for node %p\n",
			 port, endpoint, node);
		return NULL;
	}
