...
#ifndef __RTOCHIUS_XARRAY_H_
#define __RTOCHIUS_XARRAY_H_
/*
 * eXtensible Arrays
...
{
	return __dereference_check(xa->xa_head);
}

/* Private */
static inline void *xa_head_locked(const struct xarray *xa)
{
	return dereference_protected(xa->xa_head);
}
				...
				const struct xa_node *node, unsigned int offset)
{
	...
	return __dereference_check(node->slots[offset]);
}
...
{
	XA_NODE_BUG_ON(node, offset >= XA_CHUNK_SIZE);
	return dereference_protected(node->slots[offset]);
}
...
{
	return dereference_check(node->parent);
}
...
static inline struct xa_node *xa_parent_locked(const struct xarray *xa,
					const struct xa_node *node)
{
	return dereference_protected(node->parent);
}
