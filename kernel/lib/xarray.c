...
#define mark_inc(mark) do { \
	mark = (xa_mark_t)((unsigned)(mark) + 1); \
} while (0)
...
extern void __radix_tree_node_free(struct list_head *list);
...
static void *xas_alloc(struct xa_state *xas, unsigned int shift)
{
	...
	if (node) {
		...
		node = kmem_cache_alloc(radix_tree_node_cachep, __GFP_NOWARN);
		if (!node) {
	...
	node->count = 0;
	node->nr_values = 0;
	INIT_POINTER(node->parent, xas->xa_node);
	node->array = xas->xa;
...
static void xas_shrink(struct xa_state *xas)
{
	...
	for (;;) {
		...
		node->nr_values = 0;
		if (!xa_is_node(entry))
			INIT_POINTER(node->slots[0], XA_RETRY_ENTRY);
		xas_update(xas, node);
		xa_node_free(node);
		if (!xa_is_node(entry))
			break;
...
static void xas_free_nodes(struct xa_state *xas, struct xa_node *top)
{
	...
	for (;;) {
		...
		if (entry)
			INIT_POINTER(node->slots[offset], XA_RETRY_ENTRY);
		offset++;
...
static int xas_expand(struct xa_state *xas, void *head)
{
	...
	while (max > max_index(head)) {
		...
		if (xa_is_value(head))
			node->nr_values = 1;
		INIT_POINTER(node->slots[0], head);
		...
		if (xa_is_node(head)) {
			...
			assign_pointer(xa_to_node(head)->parent, node);
		}
		head = xa_mk_node(node);
		assign_pointer(xa->xa_head, head);
		xas_update(xas, node);
...
static void *xas_create(struct xa_state *xas, bool allow_root)
{
	...
	while (shift > order) {
		...
		if (!entry) {
			...
			if (xa_track_free(xa))
				node_mark_all(node, XA_FREE_MARK);
			assign_pointer(*slot, xa_mk_node(node));
