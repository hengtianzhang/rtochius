...
function(service_link_interface_ifdef feature_toggle interface)
	if(${${feature_toggle}})
		target_link_libraries(${interface} INTERFACE service_interface)
	endif()
endfunction()
...
function(service_include_directories)
	foreach(arg ${ARGV})
		if(IS_ABSOLUTE ${arg})
			set(path ${arg})
		else()
		...
		target_include_directories(service_interface INTERFACE ${path})
	endforeach()
endfunction()
...
function(service_system_include_directories)
	foreach(arg ${ARGV})
		if(IS_ABSOLUTE ${arg})
			set(path ${arg})
		else()
		...
		target_include_directories(service_interface SYSTEM INTERFACE ${path})
	endforeach()
endfunction()
...
function(service_compile_definitions)
	target_compile_definitions(service_interface INTERFACE ${ARGV})
endfunction()
...
function(service_compile_definitions_ifdef feature_toggle)
	if(${${feature_toggle}})
		service_compile_definitions(${ARGN})
	endif()
endfunction()
...
function(service_compile_definitions_ifndef feature_toggle)
	if(NOT ${${feature_toggle}})
		service_compile_definitions(${ARGN})
	endif()
endfunction()
...
function(service_compile_options)
	target_compile_options(service_interface INTERFACE ${ARGV})
endfunction()
...
function(service_compile_options_ifdef feature_toggle)
	if(${${feature_toggle}})
		service_compile_options(${ARGN})
	endif()
endfunction()
...
function(service_compile_options_ifndef feature_toggle)
	if(NOT ${${feature_toggle}})
		service_compile_options(${ARGN})
	endif()
endfunction()
...
function(service_link_libraries)
	target_link_libraries(service_interface INTERFACE ${ARGV})
endfunction()
...
function(service_link_libraries_ifdef feature_toggle)
	if(${${feature_toggle}})
		service_link_libraries(${ARGN})
	endif()
endfunction()
...
function(service_link_libraries_ifndef feature_toggle)
	if(NOT ${${feature_toggle}})
		service_link_libraries(${ARGN})
	endif()
endfunction()
...
function(service_cc_option)
	foreach(arg ${ARGV})
		target_cc_option(service_interface INTERFACE ${arg})
	endforeach()
endfunction()
...
function(service_cc_option_ifdef feature_toggle)
	if(${${feature_toggle}})
		service_cc_option(${ARGN})
	endif()
endfunction()
...
function(service_cc_option_ifndef feature_toggle)
	if(NOT ${${feature_toggle}})
		service_cc_option(${ARGN})
	endif()
endfunction()
...
function(service_cc_option_fallback option1 option2)
		target_cc_option_fallback(service_interface INTERFACE ${option1} ${option2})
endfunction()
...
function(service_ld_options)
		target_ld_options(service_interface INTERFACE ${ARGV})
endfunction()
...
function(service_ld_option_ifdef feature_toggle)
	if(${${feature_toggle}})
		service_ld_options(${ARGN})
	endif()
endfunction()
...
function(service_ld_option_ifndef feature_toggle)
	if(NOT ${${feature_toggle}})
		service_ld_options(${ARGN})
	endif()
endfunction()
...
function(service_get_include_directories_for_lang_as_string lang i)
	service_get_include_directories_for_lang(${lang} list_of_flags DELIMITER " " ${ARGN})
...
function(service_get_system_include_directories_for_lang_as_string lang i)
	service_get_system_include_directories_for_lang(${lang} list_of_flags DELIMITER " " ${ARGN})
...
function(service_get_compile_definitions_for_lang_as_string lang i)
	service_get_compile_definitions_for_lang(${lang} list_of_flags DELIMITER " " ${ARGN})
...
function(service_get_compile_options_for_lang_as_string lang i)
	service_get_compile_options_for_lang(${lang} list_of_flags DELIMITER " ")
...
function(service_get_include_directories_for_lang lang i)
	rtochius_get_parse_args(args ${ARGN})
	get_property(flags TARGET service_interface PROPERTY INTERFACE_INCLUDE_DIRECTORIES)
...
function(service_get_raw_include_directories_for_lang lib_interface lang i)
	rtochius_get_parse_args(args ${ARGN})
...
function(service_get_system_include_directories_for_lang lang i)
	rtochius_get_parse_args(args ${ARGN})
	get_property(flags TARGET service_interface PROPERTY INTERFACE_SYSTEM_INCLUDE_DIRECTORIES)
...
function(service_get_compile_definitions_for_lang lang i)
	rtochius_get_parse_args(args ${ARGN})
	get_property(flags TARGET service_interface PROPERTY INTERFACE_COMPILE_DEFINITIONS)
...
function(service_get_compile_options_for_lang lang i)
	rtochius_get_parse_args(args ${ARGN})
	get_property(flags TARGET service_interface PROPERTY INTERFACE_COMPILE_OPTIONS)
...
macro(rtochius_import_libs_boilerplate lib_name)
	...
	function(${lib_name}_user_import_libraries)
		...
		if(NOT DEFINED USER_IMPORTED_${lib_name}_PROERTY)
			...
			target_link_libraries(${lib_name} PUBLIC service_interface)
		endif()
	endfunction()
endmacro()
